...
The experiment does not send information backward in time, and popular retellings get this wrong on a loop. Erasing the tag *regroups* already-recorded data — it changes the joint distribution between main screen and which-path channel, and interference appears in the subset selected by the new condition.
...
That is the engineering content of collapse. A quantum system behaves as a distributed statistical structure in which the choice of measurement determines which joint distributions are accessible to us and which are not, rather than as "reality waiting for an observer."
